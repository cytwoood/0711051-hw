...
void leadertoworld2D(geometry_msgs::Point &follower_goal, turtlesim::Pose &leader)
{
	...
	float l_x =leader.x;
	float l_y = leader.y;
	follower_goal.x = cos(leader.theta)*temp_x - sin(leader.theta)*temp_y + l_x;
	follower_goal.y = sin(leader.theta)*temp_x +cos(leader.theta)*temp_y + l_y;
...
void Positioncontrol(geometry_msgs::Point &goal, turtlesim::Pose &follower, geometry_msgs::Twist &vel_msg)
 {
	...
	vel_msg.angular.z = 2*error_theta; 
	...
	{
	vel_msg.linear.x =0;
	vel_msg.angular.z =4*error_theta;
	}
	}

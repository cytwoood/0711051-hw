...
def quad_sim():
    ...
    while True:
        ...
        while t <= T:
            ...
            axis = 0.5*(rel_R-np.linalg.inv(rel_R))
            axis = vee_map(axis)

            # get the rotation angle of the rotation axis
            
            angle_of_axis = acos(0.5*(trace(rel_R)-1))
            
            # get the rotation error 
            if int(np.linalg.norm(axis)) == 0:

                rotation_error = [0,0,0] 
            else:
                rotation_error = angle_of_axis*(axis/np.linalg.norm(axis))
